    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <Label>
              Monto * <span className="text-xs text-muted-foreground">(positivo = a pagar, negativo = a cobrar)</span>
            </Label>
            <div className="flex gap-2 items-center">
              ...
              />
              {signo === "pagar" && (
                <Badge className="bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-300 whitespace-nowrap">
                  A PAGAR
                </Badge>
              )}
              {signo === "cobrar" && (
                <Badge className="bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300 whitespace-nowrap">
                  A COBRAR
                </Badge>
              )}
            </div>
          ...
          <div>
            ...
            <Select
              value={form.factura_id || "none"}
              onValueChange={(v) => setForm({ ...form, factura_id: v === "none" ? null : v })}
            >
              <SelectTrigger>
              ...
              <SelectContent>
                ...
                {facturas?.map((f: any) => (
                  <SelectItem key={f.id} value={f.id}>
                    {f.tipo_comprobante} {String(f.punto_venta || 0).padStart(4, "0")}-
                    {String(f.numero_comprobante || 0).padStart(8, "0")} — ${Number(f.importe_total).toLocaleString()}
                  </SelectItem>
                ))}
              </SelectContent>
            </Select>
