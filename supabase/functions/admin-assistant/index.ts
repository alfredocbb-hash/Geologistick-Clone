...
serve(async (req) => {
  ...
  try {
    ...
    const [sucursalesRes, zonasRes, tarifasRes, tenantRes, configSeguroRes] = await Promise.all([
    ...
    const configSeguro = configSeguroRes.data;
    ...
    const systemPrompt = `Eres el asistente de soporte técnico de la plataforma de logística. Tu rol es ayudar a los administradores a diagnosticar problemas de configuración, responder dudas sobre funcionalidades y guiar en la resolución de incidencias.
